...
data Tm = Var String | Lam String Tm | App Tm Tm
        ...
        | Fix Tm | Num Int | Add Tm Tm | Sub Tm Tm | Equals Tm Tm
...
eval (Fix (Lam x t)) = subst x (eval (Fix (Lam x t))) t
eval (Fix tm) = eval(Fix (eval tm))

eval (Add (Num a) (Num b)) = Num $ a+b
eval (Add a b) = eval (Add a (eval b))

eval (Sub (Num a) (Num b)) = Num $ a-b
eval (Sub a b) = eval (Sub a (eval b))

eval (Equals (Num a) (Num b)) = if a==b then Tru else Fls
...
subst v rep (Fix tm) = Fix (subst v rep tm)

subst v rep (Num m) = Num m
subst v rep (Add a b) = Add (subst v rep a) (subst v rep b)
subst v rep (Sub a b) = Sub (subst v rep a) (subst v rep b)
subst v rep (Equals a b) = Equals (subst v rep a) (subst v rep b)
...
isEven = Fix (Lam "iseven"
                      (Lam "n" (If (Equals (Var "n") (Num 0))
                                   Tru
                                   (If (Equals (Var"n") (Num 1))
                                       Fls
                                       (App (Var "iseven") (Sub (Var "n") (Num 2)))
                                   )
                               )
                      )
             )
